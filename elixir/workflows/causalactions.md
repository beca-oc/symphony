...
tracker:
  kind: linear
  project_slug: "causalactions-97e4e54efc6d"
  active_states:
    - Todo
    - In Progress
    - Merging
...
polling:
  interval_ms: 60000
workspace:
  root: ~/code/symphony-workspaces/causalactions
repo:
  name: causalactions
  github_repo: Subconscious-ai/causalactions
  default_branch: main
hooks:
  timeout_ms: 600000
  after_create: |
    set -eu
    git clone https://github.com/Subconscious-ai/causalactions.git .
...
You are working on Linear ticket `{{ issue.identifier }}` for the `causalactions` repository.
