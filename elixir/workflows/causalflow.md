...
tracker:
  kind: linear
  project_slug: "causalflow-b33f71992bc8"
  active_states:
    - Todo
    - In Progress
    - Merging
...
polling:
  interval_ms: 60000
workspace:
  root: ~/code/symphony-workspaces/causalflow
repo:
  name: causalflow
  github_repo: Subconscious-ai/causalflow
  default_branch: main
hooks:
  timeout_ms: 600000
  after_create: |
    set -eu
    git clone https://github.com/Subconscious-ai/causalflow.git .
...
You are working on Linear ticket `{{ issue.identifier }}` for the `causalflow` repository.
