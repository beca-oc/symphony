...
repo:
  name: causalintelligence
  github_repo: Subconscious-ai/causalintelligence
  default_branch: main
hooks:
  timeout_ms: 600000
  after_create: |
...
You are working on Linear ticket `{{ issue.identifier }}` for the `causalintelligence` repository.
