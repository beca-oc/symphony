...
tracker:
  kind: linear
  project_slug: "sizzl-trustgraph-ecaca43ff1f4"
  active_states:
    - Todo
    - In Progress
    - Merging
...
polling:
  interval_ms: 60000
workspace:
  root: ~/code/symphony-workspaces/sizzl-trustgraph
repo:
  name: sizzl-trustgraph
  github_repo: Subconscious-ai/sizzl-trustgraph
  default_branch: main
hooks:
  timeout_ms: 600000
  after_create: |
    set -eu
    git clone https://github.com/Subconscious-ai/sizzl-trustgraph.git .
...
You are working on Linear ticket `{{ issue.identifier }}` for the `sizzl-trustgraph` repository.
