...
You are working on a Linear ticket `{{ issue.identifier }}` for the `market-ontology` repository.
...
11. Do not move the issue to `Human Review`. Leave it active after the workpad has local validation evidence, the draft PR link, the final commit SHA, the `symphony` label status, and any deployment/check URL when available; Symphony verifies the evidence and moves the issue.
